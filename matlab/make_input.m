...
% Initiate forcing in NetCDF file 
...
% Create mask for each mode
for n=1:Nm
    
    % Load wave speed
    c=ncread(fid.grid,'c',[1 1 n],[Inf Inf 1]);
       
    % Set depth to zero where c1=0;
    if n==1
        H(c==0)=0;
    end
    
    % Set c to zero over land
    c(H==0)=0;
    
    % Initiate masks with 1 (good/ocean) everywhere
    mask.u=ones([Nx+1 Ny]);
    mask.v=ones([Nx Ny+1]);
    mask.p=ones([Nx Ny]);
    ...
    % Write mask
    ncwrite(fid.in,'mask_u',mask.u,[1 1 n]);
...
for n=1:Nm
    ...
    % Load bottom phi value
    phi_bott=ncread(fid.grid,'phi_bott',[1 1 Nm],[Inf Inf 1]);

    % Cycle through each constituent
    for k=1:Nc
    
        ITGF.pN=ITGF.p(:,:,k).*phi_bott(2:end-1,2:end-1);

        % Smooth at grid scale 
        %ITGF.pN=AVE2D(ITGF.pN,3);         
       
        % One last check for bad forcing
        ITGF.p(isnan(ITGF.pN) | isinf(ITGF.pN))=0+ii*0;
    
        % Write to file
        ncwrite(fid.in,'ITGF_pr',real(ITGF.pN),[1 1 n k]);
        ncwrite(fid.in,'ITGF_pi',imag(ITGF.pN),[1 1 n k]);
    end
end
