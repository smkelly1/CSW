		...
		for(m=0; m<NM; m++) {		
			for (j=0; j<NY+1; j++) {
				...
				for (j=0; j<NY; j++) {
					...
					U[m][j+1][0]=U[m][j+1][NX];   //third to last point -> first point
					// doesn't work, but seems correct: U[m][j+1][0]=U[m][j+1][NX]; //third to last point -> first point
					// works, but seems wrong: U[m][j+1][0]=U[m][j+1][NX+1]; //third to last point -> first point
				}
			#endif
		}
	#endif
